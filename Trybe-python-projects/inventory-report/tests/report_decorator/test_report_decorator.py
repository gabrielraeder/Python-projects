...
def test_decorar_relatorio(products_mock):
    report = ColoredReport(SimpleReport)
    result = (
        f"{VERDE}Data de fabricação mais antiga:{RES} {AZUL}2020-09-06{RES}\n"
        f"{VERDE}Data de validade mais próxima:{RES} {AZUL}2023-09-17{RES}\n"
        f"{VERDE}Empresa com mais produtos:{RES} {VER}Target Corporation{RES}"
    )

    assert report.generate(products_mock) == result
